...
index $0.52$), suggesting that the autoencoder may capture nonlinear structure
beyond a 16-component linear projection, though part of this disagreement could
also reflect the algorithmic variance of the clustering step. To assess whether the regimes
...
We deliberately adopt a feed-forward autoencoder rather than a recurrent
(LSTM~\cite{hochreiter1997long}), convolutional~\cite{bai2018empirical}, or
attention-based~\cite{vaswani2017attention} alternative. The task here is
unsupervised: rather than forecasting the next step from a sequence, we compress
the joint distribution of an entire 26-week window into a single latent code. A
feed-forward encoder operating on the flattened window treats all time steps
jointly and is well matched to this compression objective, whereas recurrent and
convolutional architectures are designed primarily for supervised next-step
prediction and add sequential inductive biases and parameters that a
reconstruction task does not require. This keeps the model parsimonious and the
latent space directly comparable to the linear PCA baseline, which also operates
on the same flattened window.
...
that PCA cannot represent even in 16 components. We interpret this gap with some
caution: part of the disagreement could arise from the algorithmic variance of
K-Means itself --- different initializations can shift cluster boundaries even on
a fixed representation --- rather than purely from the linear/nonlinear
distinction. Cleanly separating these two sources would require a
same-representation clustering-stability baseline, which we leave to future work.
...
slightly outperforms the regime-conditioned strategy on test. These figures
should be read with statistical caution: with roughly $270$ weekly observations
in each out-of-sample split, the standard error of an annualized Sharpe
ratio~\cite{lo2002statistics} is approximately $0.44$, so none of the reported
out-of-sample Sharpe ratios is statistically distinguishable from zero. The
out-of-sample results are therefore directionally suggestive --- the sign of the
test Sharpe is negative across every configuration we try --- rather than a
precise estimate of strategy performance. The validation period consistently
collapses to two active regimes regardless of latent
dimensionality (see ablation, Table~\ref{tab:ablation}), suggesting the mode collapse
reflects structural change in the data rather than insufficient model capacity.
The strategy's failure in the test set supports the view that the post-2020
era represents a distributional shift not captured by training-period regime structure.
...
Sharpe in all three cases. Note that the Full row in Table~\ref{tab:signals} is
retrained inline under the comparison protocol rather than loaded from the handoff
checkpoint of Section~\ref{sec:strategy}; its in-sample Sharpe therefore differs
slightly ($0.33$ versus $0.23$ in Table~\ref{tab:strategy-results}), but the
out-of-sample test Sharpe ($-0.28$) is reproduced exactly. The out-of-sample breakdown is therefore not
...
internal cluster-validity optimum (Section~\ref{sec:clustering}). The latent
