...
\begin{itemize}
  ...
  \item validation: 2015--2020 (15\%),
  \item test: 2021--2026 (15\%).
\end{itemize}
...
\begin{center}
Input(780) $\rightarrow$ Dense(64) $\rightarrow$ ReLU $\rightarrow$ Latent(16)
$\rightarrow$ Dense(64) $\rightarrow$ ReLU $\rightarrow$ Output(780).
\end{center}
...
\begin{figure*}[t]
  ...
  learning: a 64-neuron encoder compresses the flattened window into a
  16-dimensional latent space $(z_1, \ldots, z_{16})$, which a symmetric 64-neuron
  decoder reconstructs back to the original dimensionality.}
...
The substantially higher test loss is expected: the test partition (2021--2026)
...
A notable observation concerns the validation period (2015--2020): in an earlier
narrow 3-dimensional autoencoder architecture, almost all validation windows collapsed
into a single regime. By widening the bottleneck to 16 dimensions, the network now
possesses sufficient capacity to correctly differentiate distinct market regimes
out-of-sample across the validation window. This cleanly resolves the mode
collapse limitation previously observed, allowing for a more robust representation.
...
16-dimensional bottleneck, we ensured the autoencoder possessed enough capacity
to meaningfully differentiate regimes out-of-sample without suffering from mode
collapse.
...
by post-2020 market conditions. This suggests that while 16 dimensions are sufficient
to model the historical dataset, historical models of any capacity struggle
to generalize across true, unprecedented regime shifts.
...
structural break seen in the 2021--2026 data. This suggests that while representation
