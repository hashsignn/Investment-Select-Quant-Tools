...
I packaged everything — windows, scaler, schema, metadata — into one handoff folder
Eleni could load in two lines of Python and start training immediately.
\end{cue}
...
Why 16 dimensions? Harjot's analysis showed a 10-component PCA already explains
67\% of variance, and many features are strongly correlated — 11 pairs above 0.8.
So the 30 inputs carry far fewer independent dimensions; 16 is a meaningful
compression without collapsing the representation. An earlier 3-D bottleneck
suffered mode collapse, which widening to 16 fixed.
\end{cue}

\begin{cue}{Training \& early stopping}{60 sec}
I trained with the \textbf{Adam} optimiser and a weight decay of $5\times10^{-4}$
for regularisation. Crucially, I used \textbf{early stopping} on the validation
loss — up to 200 epochs, but the best checkpoint is saved when validation stops
improving. This prevents overfitting to the training period.

Reconstruction loss reached about \textbf{0.30} on train, \textbf{0.61} on
validation, and \textbf{1.20} on test. The higher test loss reflects genuine
\textbf{distributional shift}: the test period (2020–2026) includes COVID, the
post-pandemic inflation surge, and aggressive Fed rate hikes — conditions barely
present in training. We read this as evidence of a possible regime shift, not a
model failure.
\end{cue}

\begin{cue}{Clustering into regimes}{60 sec}
...
Trading Sharpe: \textbf{train 0.23 / val 0.13 / test $-$0.28}.
\end{numbers}
...
\begin{cue}{Closing}{40 sec}
To summarise: Harjot built a rigorously leak-free pipeline on 38 years of weekly
data. Eleni trained a non-linear autoencoder compressing it into 16 dimensions and
clustered four regimes. I benchmarked that against a linear PCA of identical
dimensionality, found moderate agreement, and interpreted each regime
economically. The result is a fully reproducible, end-to-end pipeline for
unsupervised market regime detection — with honest documentation of where the
autoencoder adds value over the linear baseline and where it does not yet
generalise out of sample. Thank you.
\end{cue}
